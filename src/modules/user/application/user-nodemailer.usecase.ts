...
export class UserNodemailerUpdateUseCase<TDB> {
    ...
    async update(
        ...
        if(!user)throw createDomainError(ErrorCodes.DATABASE_FIND,UserNodemailerUpdateUseCase,"readById",undefined,{entity: "user", optionalMessage: "Not found at search existing for update"})
        ...
        if(userUpdateProps.email !== null && userUpdateProps.email !== undefined && user.email !== userUpdateProps.email ){
                ...
                if(!base)throw createDomainError(ErrorCodes.SET_ENV,UserNodemailerUpdateUseCase,"public base url",undefined,{function:"update"})
