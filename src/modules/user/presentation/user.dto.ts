...
import { UserBase, UserFormS } from '@skrteeeeee/profile-domain';
...
export class UserManageRoleDto {
  ...
  @ApiProperty({
    title: 'ID',
    description:
      'Unique identifier assigned to a user by the database (MongoDB ObjectId).',
...
export class UserUpdateDto implements UserNodemailerUpdateProps {
  @ApiProperty({
    title: 'User ID',
    description:
      'Unique identifier assigned to the user by the database (MongoDB ObjectId).',
    example: '665b1c2f8f1b2a0012a34567',
  })
  @IsString()
  id: string;
  ...
  @ApiProperty({
    title: 'Email',
    description:
      "User's email address. Must be a valid email format. Optional for updates.",
  ...
  @ApiProperty({
    title: 'Profile Image',
    description:
      "URL or path to the user's profile image. Optional for updates.",
...
@ApiDtoMetadata({
  description:
    'Address of the user to be logged. *This Body is only required in case of mock for facilitate not use ever the sign wallet payload*',
...
export class UserDto extends UserUpdateDto implements User {
  ...
  @ApiProperty({
    ...apiRoleType,
    description: 'Role type permission that User required',
  })
  @IsOptional()
  @IsEnum(RoleType)
