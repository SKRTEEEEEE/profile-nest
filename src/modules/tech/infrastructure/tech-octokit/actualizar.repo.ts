...
private async actualizarMd(props:{create?: ActualizarGithubTechsProps["create"], proyectosDB?: (LengBase & MongooseBase)[]}){
...
private  createBadgeTech(tech: PreTechBase, newTech?: true ) {
    const {af, afNum, exp, expNum, github, githubNum} = this.getTechsLinks({nameId: tech.nameId, color: tech.color, nameBadge:tech.nameBadge, web:tech.web})
    const baseBadge = this.createBaseBadge({nameId: tech.nameId, color: tech.color, nameBadge:tech.nameBadge, web:tech.web})
    if(newTech === true) {
...
private  getTechsLinks({nameId, nameBadge, color}: PreTechBase): GetLinksResp {
