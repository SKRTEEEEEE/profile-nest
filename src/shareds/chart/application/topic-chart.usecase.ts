...
export class TopicChartUseCase implements TopicChartInterface {
    ...
    async renderTopicAlphaBarChart(owner:string,type: "alpha-triple" | "alpha-simple", reposDetails: RepoDetailsRes): Promise<Buffer> {
        const allTopicsSize = this.topicCalculatorUseCase.calculateAllTopicSizePercentages(reposDetails);
        if (type === "alpha-triple") {
            const allTopicsRepo = this.topicCalculatorUseCase.calculateAllTopicsRepoPercentages(reposDetails);
            ...
            throw createDomainError(ErrorCodes.NOT_IMPLEMENTED, TopicChartUseCase, 'renderTopicAlphaBarChart');
        }
        throw createDomainError(ErrorCodes.INPUT_PARSE, TopicChartUseCase, 'renderTopicAlphaBarChart');
    }
