...
export class AuthThirdWebVerifyPayloadDto implements VerifyLoginPayloadParams {
  @ApiProperty({
    title: 'Signature',
    description:
      "A valid hexadecimal string representing the signature, starting with '0x'.",
    example:
  ...
  @ApiProperty({
    title: 'Payload',
    description:
      'The login payload object from thirdweb/auth, containing authentication details.',
    example: {
...
@ApiDtoMetadata({
  description:
    'Jwt auth created with your user web3 account of the application or mock (depends of the implementation)',
  title: 'Verify JWT',
  group: 'Shared',
})
export class AuthThirdWebVerifyJwtDto {
  @ApiProperty({
    title: 'Token',
    description:
      'The JSON Web Token (JWT) used for authentication, generated or mocked based on the implementation.',
    example:
