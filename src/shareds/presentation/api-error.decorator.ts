...
export function ApiErrorResponse(...codes: (ErrorCodes | "full" | "get" | "--protected" | "d")[]) {
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    if (!codes.includes("full")) {
      ...
      if (codes.includes("get")) usedCodes = [...usedCodes, ErrorCodes.DATABASE_FIND]
      if (codes.includes("--protected")) usedCodes = [...usedCodes, ErrorCodes.UNAUTHORIZED_ACTION]
      if (codes.includes("d")) {
        if(codes.length === 1) {
          usedCodes = [...usedCodes, ErrorCodes.UNAUTHORIZED_ACTION, ErrorCodes.DATABASE_ACTION]
        } else {
    ...
    const responses = usedCodes
      .filter((code): code is ErrorCodes =>
        code !== "full" &&
        code !== "get" &&
        code !== "--protected" &&
        code !== "d"
