...
export type AllTopicRepoPercentagesRes = {
  name: string;
  topicRepoPer: number;
  topicRepoFrac: string;
}[];

export type TopicGithubDataRes = {
  topicSizePer: number;
  topicRepoPer: number;
  topicRepoFrac: string;
...
export abstract class TopicCalculatorInterface {
  abstract calculateAllTopicSizePercentages(
    reposDetails: RepoDetailsRes,
  ): AllTopicSizePercentagesRes;
  abstract calculateAllTopicsRepoPercentages(
    reposDetails: RepoDetailsRes,
