...
export class JwtAuthThirdwebGuard extends AuthGuard('thirdweb') {
  ...
  canActivate(
    ...
    if (requiredRoles && isPublic) {
      console.warn(
        `[UserAuth] ⚠️ Se están usando @Roles y @PublicRoute juntos en ${context.getClass().name + '.' + context.getHandler().name}. Se ignorará @PublicRoute`,
      );
    ...
    if (err || !user) {
      throw (
        ...
        createDomainError(
          ...
          'handleRequest',
          undefined,
          { shortDesc: 'Error at handle request in jwt' },
        )
      );
    }
    return user;
